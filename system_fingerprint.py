...
logger = log_helper.setup_logger(name="system_fingerprint", level=logging.INFO, log_to_file=False)


class WinFingerprint:
    """
    Идентификаторы отпечатков пальцев, связанные с Windows, такие как издание, версия, сборка, обновления
    """
    EDITIONS = {
        7: [["Starter", "Starter"],
    ...
    BUILDS = {
        ...
        10: ["Windows 10", "10.0", "19045", "19041.1.amd64fre.vb_release.191206-1406", "19041.1.amd64fre.vb_release.191206-1406"]
    }
    ...
    def __init__(self):
        ...
        self.install_date = random_utils.random_unix_time("01.01.2015", "01.01.2024")
    ...
    def __random_digital_product_id(self):
        random_digital_id = [random.randint(0, 255) for _ in range(164)]
        random_digital_id[0x00:0x08] = [0xA4, 0, 0, 0, 0x3, 0, 0, 0]
        pid_bytes = [ord(c) for c in self.product_id]
        random_digital_id[0x08:0x08 + len(pid_bytes)] = pid_bytes
        random_digital_id[0xA0:0xA4] = [0xB9, 0xEC, 0x21, 0x73]
    ...
    def __random_product_id4():
        ...
        if len(dispersed_list) > 70:
            dispersed_list[70] = random.randint(0, 0xFF)
        if len(dispersed_list) > 2:
    ...
    def __random_digital_product_id4(self):
        ...
        random_digital_id4[0x08 : 0x08 + len(random_id1)] = random_id1
        
        # 0x88 - UUID
        product_guid_id4 = random_utils.disperse_string(self.uuid_id4)
        random_digital_id4[0x88 : 0x88 + len(product_guid_id4)] = product_guid_id4
        
        # 0x0118 - Edition
        product_edition = random_utils.disperse_string(self.edition_id)
        random_digital_id4[0x0118 : 0x0118 + len(product_edition)] = product_edition
        
        # 0x0328 - random length 80
        random_block = [random.randint(0, 255) for _ in range(80)]
        random_digital_id4[0x0328 : 0x0328 + len(random_block)] = random_block
        ...
        random_digital_id4[0x0378 : 0x0378 + len(random_id2)] = random_id2
        
        # 0x03F8 & 0x0478 - Retail/OEM
        retail_oem = random_utils.disperse_string(self.retail_oem)
        random_digital_id4[0x03F8 : 0x03F8 + len(retail_oem)] = retail_oem
        random_digital_id4[0x0478 : 0x0478 + len(retail_oem)] = retail_oem
